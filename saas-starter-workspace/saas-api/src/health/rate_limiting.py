"""Project shim exposing vendor health helpers for rate_limiting."""
...
_VENDOR_RELATIVE_PATH = "src/health/rate_limiting.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_rate_limiting"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/rate-limiting"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_rate_limiting_health expects a FastAPI application instance"
        )
...
try:
    register_rate_limiting_health = _resolve_export("register_rate_limiting_health")
except Exception:
    register_rate_limiting_health = _fallback_register
