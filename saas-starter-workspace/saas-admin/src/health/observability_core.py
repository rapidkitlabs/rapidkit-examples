"""Project shim exposing vendor health helpers for observability_core."""
...
_VENDOR_MODULE = "observability_core"
_VENDOR_VERSION = "0.1.15"
_VENDOR_RELATIVE_PATH = "src/health/observability_core.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_observability_core"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/observability-core"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_observability_core_health expects a FastAPI application instance"
        )
...
try:
    register_observability_core_health = _resolve_export("register_observability_core_health")
except Exception:
    register_observability_core_health = _fallback_register
