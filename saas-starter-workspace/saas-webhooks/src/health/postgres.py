"""Project shim exposing vendor health helpers for postgres."""
...
DEFAULT_HEALTH_PREFIX = "/api/health/module/postgres"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_postgres_health expects a FastAPI application instance"
        )
...
try:
    register_postgres_health = _resolve_export("register_postgres_health")
except Exception:
    register_postgres_health = _fallback_register
