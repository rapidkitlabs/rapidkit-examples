"""Project shim exposing vendor health helpers for middleware."""
...
DEFAULT_HEALTH_PREFIX = "/api/health/module/middleware"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_middleware_health expects a FastAPI application instance"
        )
...
try:
    register_middleware_health = _resolve_export("register_middleware_health")
except RuntimeError:
    register_middleware_health = _fallback_register
