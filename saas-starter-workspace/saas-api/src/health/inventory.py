"""Project shim exposing vendor health helpers for inventory."""
...
_VENDOR_MODULE = "inventory"
_VENDOR_VERSION = "0.1.11"
_VENDOR_RELATIVE_PATH = "src/health/inventory.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_inventory"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/inventory"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_inventory_health expects a FastAPI application instance"
        )
...
try:
    register_inventory_health = _resolve_export("register_inventory_health")
except Exception:
    register_inventory_health = _fallback_register
