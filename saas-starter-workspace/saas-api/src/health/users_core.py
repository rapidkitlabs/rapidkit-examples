"""Project shim exposing vendor health helpers for users_core."""
...
_VENDOR_RELATIVE_PATH = "src/health/users_core.py"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_users_core_health expects a FastAPI application instance"
        )
...
try:
    register_users_core_health = _resolve_export("register_users_core_health")
except Exception:
    register_users_core_health = _fallback_register
