"""Project shim exposing vendor health helpers for users_profiles."""
...
DEFAULT_HEALTH_PREFIX = "/api/health/module/users-profiles"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_users_profiles_health expects a FastAPI application instance"
        )
...
try:
    register_users_profiles_health = _resolve_export("register_users_profiles_health")
except Exception:
    register_users_profiles_health = _fallback_register
