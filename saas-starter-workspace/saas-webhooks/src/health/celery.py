"""Project shim exposing vendor health helpers for celery."""
...
_VENDOR_RELATIVE_PATH = "src/health/celery.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_celery"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/celery"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_celery_health expects a FastAPI application instance"
        )
...
try:
    register_celery_health = _resolve_export("register_celery_health")
except Exception:
    register_celery_health = _fallback_register
