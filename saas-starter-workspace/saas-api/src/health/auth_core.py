"""Project shim exposing vendor health helpers for auth_core."""
...
DEFAULT_HEALTH_PREFIX = "/api/health/module/auth-core"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_auth_core_health expects a FastAPI application instance"
        )
...
try:
    register_auth_core_health = _resolve_export("register_auth_core_health")
except Exception:
    register_auth_core_health = _fallback_register
