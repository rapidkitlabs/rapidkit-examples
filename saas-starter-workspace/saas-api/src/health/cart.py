"""Project shim exposing vendor health helpers for cart."""
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_cart_health expects a FastAPI application instance"
        )
...
try:
    register_cart_health = _resolve_export("register_cart_health")
except Exception:
    register_cart_health = _fallback_register
