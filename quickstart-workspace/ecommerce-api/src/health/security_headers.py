"""Project shim exposing vendor health helpers for security_headers."""
...
DEFAULT_HEALTH_PREFIX = "/api/health/module/security-headers"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_security_headers_health expects a FastAPI application instance"
        )
...
try:
    register_security_headers_health = _resolve_export("register_security_headers_health")
except Exception:
    register_security_headers_health = _fallback_register
