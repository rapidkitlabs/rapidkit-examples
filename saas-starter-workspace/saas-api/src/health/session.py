"""Project shim exposing vendor health helpers for session."""
...
_VENDOR_MODULE = "session"
_VENDOR_VERSION = "0.1.14"
_VENDOR_RELATIVE_PATH = "src/health/session.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_session"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/session"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_session_health expects a FastAPI application instance"
        )
...
try:
    register_session_health = _resolve_export("register_session_health")
except Exception:
    register_session_health = _fallback_register
