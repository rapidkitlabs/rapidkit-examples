"""Project shim exposing vendor health helpers for ai_assistant."""
...
DEFAULT_HEALTH_PREFIX = "/api/health/module/ai-assistant"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_ai_assistant_health expects a FastAPI application instance"
        )
...
try:
    register_ai_assistant_health = _resolve_export("register_ai_assistant_health")
except RuntimeError:
    register_ai_assistant_health = _fallback_register
