"""Project shim exposing vendor health helpers for logging."""
...
_VENDOR_RELATIVE_PATH = "src/health/logging.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_logging"

DEFAULT_HEALTH_PREFIX = "/api/health/module/logging"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_logging_health expects a FastAPI application instance"
        )
...
try:
    register_logging_health = _resolve_export("register_logging_health")
except RuntimeError:
    register_logging_health = _fallback_register
