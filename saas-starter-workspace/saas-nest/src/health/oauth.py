"""Project shim exposing vendor health helpers for oauth."""
...
_VENDOR_MODULE = "oauth"
_VENDOR_VERSION = "0.1.16"
_VENDOR_RELATIVE_PATH = "src/health/oauth.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_oauth"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/oauth"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_oauth_health expects a FastAPI application instance"
        )
...
try:
    register_oauth_health = _resolve_export("register_oauth_health")
except Exception:
    register_oauth_health = _fallback_register
