"""Project shim exposing vendor health helpers for stripe_payment."""
...
_VENDOR_RELATIVE_PATH = "src/health/stripe_payment.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_stripe_payment"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/stripe-payment"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_stripe_payment_health expects a FastAPI application instance"
        )
...
try:
    register_stripe_payment_health = _resolve_export("register_stripe_payment_health")
except Exception:
    register_stripe_payment_health = _fallback_register
