"""Project shim exposing vendor health helpers for email."""
...
_VENDOR_RELATIVE_PATH = "src/health/email.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_email"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/email"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_email_health expects a FastAPI application instance"
        )
...
try:
    register_email_health = _resolve_export("register_email_health")
except Exception:
    register_email_health = _fallback_register
