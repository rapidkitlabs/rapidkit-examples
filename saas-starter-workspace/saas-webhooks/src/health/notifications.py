"""Project shim exposing vendor health helpers for notifications."""
...
_VENDOR_MODULE = "notifications"
_VENDOR_VERSION = "0.1.31"
_VENDOR_RELATIVE_PATH = "runtime/communication/notifications_health.py"
_VENDOR_ROOT_ENV = "RAPIDKIT_VENDOR_ROOT"
_CACHE_PREFIX = "rapidkit_vendor_notifications"
_STARTED_AT = time.monotonic()

DEFAULT_HEALTH_PREFIX = "/api/health/module/notifications"
...
def _fallback_register(app: Any) -> None:
    ...
    if FastAPI is not None and not isinstance(app, FastAPI):
        raise TypeError(
            "register_notifications_health expects a FastAPI application instance"
        )
...
try:
    register_notifications_health = _resolve_export("register_notifications_health")
except Exception:
    register_notifications_health = _fallback_register
